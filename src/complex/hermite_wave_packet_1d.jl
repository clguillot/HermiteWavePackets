...
function (H::HermiteWavePacket1D)(x::Number)
...
function evaluate(H::HermiteWavePacket1D, x::SVector{M, <:Number}) where M
    Ha = HermiteFct1D(H.Λ, real(H.z), H.q)
    e = @. cis(-imag(H.z) * (x - H.q)^2 / 2) * cis(x * H.p)
...
#=
    Computes the product of a hermite wave packet with a polynomial
        P(x) = ∑ₖ P[k](x-q)^k
=#
function polynomial_product(q::Real, P::SVector{N, <:Number}, H::HermiteWavePacket1D) where N
    Ha = HermiteFct1D(H.Λ, real.(H.z), H.q)
...
@inline function unitary_product(b::Real, q::Real, p::Real, H::HermiteWavePacket1D)
...
@inline function norm2_L2(H::HermiteWavePacket1D)
